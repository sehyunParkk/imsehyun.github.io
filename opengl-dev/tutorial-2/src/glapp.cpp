...
{
	int const count = (slices + 1) * (stacks + 1);
	std::vector<glm::vec2> pos_vtx(count);
	std::vector<glm::vec3> clr_vtx(count);
	...
	for (int row = 0; row < stacks; ++row)
	{
		for (int col = 0; col <= slices; ++col)
		{
			idx_vtx.push_back(static_cast<GLushort>(((slices + 1) * (row + 1)) + col));
			idx_vtx.push_back(static_cast<GLushort>(((slices + 1) * row) + col));
		}

		if (row < stacks - 1)
		{
			idx_vtx.push_back(static_cast<GLushort>(slices + (slices + 1) * row));
			idx_vtx.push_back(static_cast<GLushort>(((slices + 1) * (row + 2))));
		}
	}
