...
bool GLHelper::init(GLint w, GLint h, std::string t) {
  ...
  // Part 1
  if (!glfwInit()) { // GLFW 라이브러리 초기화 --> 성공적으로 초기화가 되면 GL_TRUE 반환
    std::cout << "GLFW init has failed - abort program!!!" << std::endl; // 초기화가 되지 않음
    return false;
  }

  // In case a GLFW function fails, an error is reported to callback function
  glfwSetErrorCallback(GLHelper::error_cb); // GLFW 함수가 실패했을 때 콜백되는 함수
